# -------------------------------------------------------------
# Logistic Regression on Environment as a Priority
# -------------------------------------------------------------
...
# 3. Create binary dependent variable for Environment priority
# Environment = 11 in De Stemming survey question

df <- df %>%
  mutate(Environment_Priority = ifelse(IP_Closed_1 == 11 | IP_Closed_2 == 11, 1, 0))
...
model_environment <- glm(Environment_Priority ~ Female_Effect + Parent_Effect + 
...
# 7. Tidy the results
tidy(model_environment, exponentiate = TRUE, conf.int = TRUE)
