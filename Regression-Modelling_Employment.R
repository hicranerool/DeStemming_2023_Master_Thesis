# -------------------------------------------------------------
# Logistic Regression on Employment as a Priority
# -------------------------------------------------------------
...
# 3. Create binary dependent variable for Employment priority
# Employment = 15 in De Stemming survey question

df <- df %>%
  mutate(Employment_Priority = ifelse(IP_Closed_1 == 15 | IP_Closed_2 == 15, 1, 0))
...
model_employment <- glm(Employment_Priority ~ Female_Effect + Parent_Effect + 
...
# 7. Tidy the results
tidy(model_employment, exponentiate = TRUE, conf.int = TRUE)
